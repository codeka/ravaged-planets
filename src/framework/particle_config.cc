...
namespace fw {
	...
	{
	private:
		typedef std::map<std::string, shared_ptr<particle_effect_config> > config_map;
		config_map _configs;
	public:
		shared_ptr<particle_effect_config> get_config(std::string const &name);
		void add_config(std::string const &name, shared_ptr<particle_effect_config> config);
	};

	shared_ptr<particle_effect_config> particle_effect_config_cache::get_config(std::string const &name)
	{
		config_map::iterator it = _configs.find(name);
		if (it == _configs.end())
			return shared_ptr<particle_effect_config>();

		return it->second;
	}

	void particle_effect_config_cache::add_config(std::string const &name, shared_ptr<particle_effect_config> config)
	{
	...
	shared_ptr<particle_effect_config> particle_effect_config::load(std::string const &name)
	{
		fs::path filepath;
		...
		else
		{
			...
			shared_ptr<particle_effect_config> config = g_cache.get_config(name);
			if (config)
			{
			...
			filepath = fw::installed_data_path() / "particles" / (name + ".wwpart");
		}
		fw::xml_element xmldoc = fw::load_xml(filepath, "wwparticle", 1);

		shared_ptr<particle_effect_config> config(new particle_effect_config());
			...
			for(xml_element child = root.get_first_child(); child.is_valid(); child = child.get_next_sibling())
			{
				...
				else
				{
					BOOST_THROW_EXCEPTION(fw::exception()
						...
						<< fw::message_error_info("unknown child element of <wwparticle>"));
				}
			}
	...
	{
		shared_ptr<particle_emitter_config> emitter_config(new particle_emitter_config());
	...
	particle_emitter_config::particle_emitter_config()
	{
		...
		billboard.texture = shared_ptr<texture>();
	...
	void particle_emitter_config::load_emitter(xml_element const &emitter_elem)
	{
		...
		for(xml_element child = emitter_elem.get_first_child(); child.is_valid(); child = child.get_next_sibling())
		{
			...
			else
			{
				BOOST_THROW_EXCEPTION(fw::exception()
					<< fw::xml_error_info(child)
					<< fw::message_error_info("unknown child element of <emitter>"));
			}
		}
	}
	...
	void particle_emitter_config::load_position(xml_element const &elem)
	{
		...
		if (elem.is_attribute_defined("falloff"))
		{
			...
			if (value == "linear")
				...
				BOOST_THROW_EXCEPTION(fw::exception() << fw::xml_error_info(elem) << fw::message_error_info("unknown 'falloff' attribute value"));
		}
	}
	...
	void particle_emitter_config::load_billboard(xml_element const &elem)
	{
		...
		texture->create(fw::installed_data_path() / "particles" / filename);
		...
		if (elem.is_attribute_defined("mode"))
		{
			...
			else if (mode != "normal")
			{
				BOOST_THROW_EXCEPTION(fw::exception()
					<< fw::xml_error_info(elem)
					<< fw::message_error_info("Billboard mode must be 'additive' or 'normal'"));
			}
		}
		...
		for(fw::xml_element child = elem.get_first_child(); child.is_valid(); child = child.get_next_sibling())
		{
			if (child.get_value() == "area")
			{
				...
				if (components.size() != 4)
				{
					BOOST_THROW_EXCEPTION(fw::exception()
						<< fw::xml_error_info(elem)
						<< fw::message_error_info("rect values require 4 floating point values"));
				}
			...
			else
			{
				BOOST_THROW_EXCEPTION(fw::exception()
					<< fw::xml_error_info(elem)
					<< fw::message_error_info("Unknown child of 'billboard'."));
			}
		}
	}
	...
	void particle_emitter_config::load_life(xml_element const &elem)
	{
		...
		for(xml_element child = elem.get_first_child(); child.is_valid(); child = child.get_next_sibling())
		{
			...
			else
			{
				BOOST_THROW_EXCEPTION(fw::exception()
					<< fw::xml_error_info(child)
					<< fw::message_error_info("unknown child element of <life>"));
			}
		}
	}
	...
	void particle_emitter_config::parse_life_state(life_state &state, xml_element const &elem)
	{
		...
		for(xml_element child = elem.get_first_child(); child.is_valid(); child = child.get_next_sibling())
		{
			...
			else if (child.get_value() == "rotation")
			{
				if (child.is_attribute_defined("kind"))
				{
					...
					else if (kind != "random")
					{
						BOOST_THROW_EXCEPTION(fw::exception()
							<< fw::xml_error_info(child)
							<< fw::message_error_info("kind expected to be 'random' or 'direction'"));
					}
				}
			...
			else
			{
				BOOST_THROW_EXCEPTION(fw::exception()
					<< fw::xml_error_info(child)
					<< fw::message_error_info("unknown child element of <state>"));
			}
		}
	}
	...
	void particle_emitter_config::parse_random_colour(random<fw::colour> &value, xml_element const &elem)
	{
		...
		else
		{
			BOOST_THROW_EXCEPTION(fw::exception()
				<< fw::xml_error_info(elem)
				<< fw::message_error_info("colour values require 3 or 4 floating point values"));
		}
	...
	void particle_emitter_config::parse_random_vector(random<fw::vector> &value, xml_element const &elem)
	{
		...
		else
		{
			BOOST_THROW_EXCEPTION(fw::exception()
				<< fw::xml_error_info(elem)
				<< fw::message_error_info("values require 3 or 4 floating point values"));
		}
