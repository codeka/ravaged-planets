...
namespace ww {
	...
	{
		std::string url = (boost::format("Api/Session/New?name=%1%&password=%2%&listenPort=%3%")
								% _username % _password % _listen_port).str();
	...
	{
		return (boost::format("<game sessionId=\"%1%\" />")
			% _session_id).str();
	}
	...
	bool create_game_session_request::parse_response(fw::xml_element &xml)
	{
		if (xml.get_value() != "success")
		{
			fw::debug << boost::format("ERR: unexpected response from \"join game\" request: %1%") % xml.get_value() << std::endl;
			return false;
		}
		...
		fw::debug << boost::format("new game registered, identifier: %1%")
				% game_id << std::endl;
	...
	bool list_games_session_request::parse_response(fw::xml_element &xml)
	{
		...
		for(fw::xml_element game = xml.get_first_child();
			game.is_valid();
			game = game.get_next_sibling())
		{
			remote_game g;
	...
	{
		return (boost::format("<join sessionId=\"%1%\" gameId=\"%2%\" />")
			% _session_id % _game_id).str();
	}
	...
	bool join_game_session_request::parse_response(fw::xml_element &xml)
	{
		...
		fw::debug << boost::format("joined game, address: %1% player# %2%")
				% server_address % static_cast<int>(player_no) << std::endl;
	...
	{
		return (boost::format("<confirm sessionId=\"%1%\" gameId=\"%2%\" otherUserId=\"%3%\" />")
			% _session_id % _game_id % _other_user_id).str();
	}
	...
	bool confirm_player_session_request::parse_response(fw::xml_element &xml)
	{
		if (xml.get_value() != "success")
		{
			fw::debug << boost::format("ERR: unexpected response from \"confirm player\" request: %1%") % xml.get_value() << std::endl;
			return false;
		}
		...
		if (confirmed == "true")
		{
			...
			fw::debug << boost::format("connecting player confirmed, user-id: %1%, username: %2%, player#: %3%")
				% _other_user_id % _other_user_name % static_cast<int>(_player_no) << std::endl;
		}
		else
		{
			fw::debug << boost::format("connecting player is not logged in to server, not allowing! user-id: %1%") % _other_user_id << std::endl;
			_confirmed = false;
		}
