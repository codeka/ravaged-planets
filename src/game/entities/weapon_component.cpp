...
namespace ent {
	...
	void weapon_component::apply_template(shared_ptr<entity_component_template> comp_template)
	{
	...
	void weapon_component::update(float dt)
	{
		...
		shared_ptr<ent::entity> target = _target.lock();
		if (target)
		{
			...
			if (our_pos == 0 || their_pos == 0)
				return;

			if (our_moveable != 0)
			{
				float wrap_x = ww::world::get_instance()->get_terrain()->get_width();
				float wrap_z = ww::world::get_instance()->get_terrain()->get_length();
	...
	void weapon_component::fire()
	{
		shared_ptr<ent::entity> entity(_entity);
		// TODO: entity_id
		shared_ptr<ent::entity> ent = entity->get_manager()->create_entity(entity, _fire_entity_name, 0);
