...
namespace ent {
	...
	void moveable_component::apply_template(shared_ptr<entity_component_template> comp_template)
	{
	...
	void moveable_component::initialise()
	{
	...
	void moveable_component::set_goal(fw::vector goal)
	{
		...
		// make sure we constraing the goal to the bounds of the map
		_goal = fw::vector(
	...
	void moveable_component::update(float dt)
	{
		...
		{
			shared_ptr<ent::entity> obstacle = _pos->get_nearest_entity().lock();
			if (obstacle)
			{
				...
				{
					// if they're selectable, reduce the distance by their selection radius - that's
					// what we ACTUALLY want to avoid...
					float obstacle_radius = 1.0f;
					selectable_component *their_selectable = obstacle->get_component<selectable_component>();
					if (their_selectable != 0)
					{
					...
					if (obstacle_distance < (obstacle_radius * 2.0f) && obstacle_distance < distance)
					{
						...
						if (show_steering)
						{
							...
							// to avoid it.
							entity->get_debug_view()->add_line(obstacle_pos,
								obstacle_pos + (avoid_dir * (obstacle_radius * 2.0f)), fw::colour(0, 0, 1));
						}
	...
	fw::vector moveable_component::steer(fw::vector pos, fw::vector curr_direction,
