...
namespace ent {
	...
	void projectile_component::apply_template(shared_ptr<entity_component_template> comp_template)
	{
	}

	void projectile_component::initialise()
	{
	...
	void projectile_component::set_target(weak_ptr<entity> target)
	{
		_target = target;

		shared_ptr<entity> sp = target.lock();
		if (sp)
	...
	void projectile_component::update(float)
	{
		...
		// then we've hit them!
		shared_ptr<ent::entity> nearest = _our_position->get_nearest_entity_with_component<damageable_component>().lock();
		shared_ptr<ent::entity> creator = entity->get_creator().lock();
		...
		if (!exploded)
		{
			...
			ww::terrain *trn = ww::world::get_instance()->get_terrain();

			fw::vector pos = _our_position->get_position();
			float height = trn->get_height(pos[0], pos[2]);
			if (height >= pos[1])
			{
				explode(shared_ptr<ent::entity>());
				exploded = true;
			}
		}
	}

	void projectile_component::explode(shared_ptr<entity> hit)
	{
	...
	void seeking_projectile_component::apply_template(shared_ptr<entity_component_template> comp_template)
	{
	...
	void seeking_projectile_component::update(float dt)
	{
		...
		shared_ptr<entity> target = _target.lock();
	...
	void ballistic_projectile_component::apply_template(shared_ptr<entity_component_template> comp_template)
	{
		BOOST_FOREACH(entity_component_template::property_map::value_type &kvp, comp_template->properties)
		{
	...
	void ballistic_projectile_component::set_target(weak_ptr<entity> target)
	{
		projectile_component::set_target(target);
		shared_ptr<entity> sptarget = target.lock();
		if (!sptarget)
			return;
