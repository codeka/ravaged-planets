...
namespace ent {
	...
	void entity_factory::populate(shared_ptr<entity> ent, std::string name)
	{
		...
		shared_ptr<entity_template> &entity_template = it->second;
		...
		BOOST_FOREACH(entity_template::component_template_list::value_type comp_template, entity_template->components)
		{
	...
	void entity_factory::get_templates(std::vector< shared_ptr<entity_template> > &templates)
	{
		BOOST_FOREACH(entity_template_map::value_type &kvp, *entity_templates)
		{
			shared_ptr<entity_template> &ent_template = kvp.second;
			templates.push_back(ent_template);
		}
	}

	shared_ptr<entity_template> entity_factory::get_template(std::string name)
	{
		entity_template_map::iterator it = entity_templates->find(name);
		if (it == entity_templates->end())
		{
			return shared_ptr<entity_template>();
		}
	...
	void entity_factory::get_buildable_templates(std::string const &build_group, std::vector< shared_ptr<entity_template> > &templates)
	{
		BOOST_FOREACH(entity_template_map::value_type &kvp, *entity_templates)
		{
			shared_ptr<entity_template> &ent_template = kvp.second;
			BOOST_FOREACH(entity_template::component_template_list::value_type comp_template, ent_template->components)
			{
	...
	// loads all of the *.wwentity files in the .\data\entities folder one by one, and
	...
	void entity_factory::load_entities()
	{
		...
		{
			if (fs::is_regular_file(it->status()) && it->path().extension() == ".wwentity")
			{
				fw::xml_element root = fw::load_xml(it->path(), "wwentity", 1);
				shared_ptr<entity_template> ent_template(new entity_template());

				std::string ent_template_name = it->path().stem();
	...
	void entity_factory::load_document(shared_ptr<entity_template> ent_template, fw::xml_element const &root)
	{
		for(fw::xml_element elem = root.get_first_child();
			elem.is_valid(); elem = elem.get_next_sibling())
		{
			...
			{
				fw::debug << boost::format("  warning: skipping unknown child element \"%1%\" (line: %2% col: %3%)")
					% elem.get_value() % elem.get_element()->Row() % elem.get_element()->Column() << std::endl;
			}
		}
	}

	void entity_factory::load_components(shared_ptr<entity_template> ent_template, fw::xml_element const &elem)
	{
		for(fw::xml_element child = elem.get_first_child();
			child.is_valid(); child = child.get_next_sibling())
		{
			...
			{
				fw::debug << boost::format("  warning: skipping unknown child element \"%1%\" (line: %2% col: %3%)")
					% child.get_value() % child.get_element()->Row() % child.get_element()->Column() << std::endl;
			}
		}
	}

	void entity_factory::load_component(shared_ptr<entity_template> ent_template, fw::xml_element const &elem)
	{
		...
		shared_ptr<entity_component_template> comp_template(comp->create_template());
	...
	void entity_factory::load_attributes(shared_ptr<entity_template> ent_template, fw::xml_element const &elem)
	{
		for(fw::xml_element child = elem.get_first_child();
			child.is_valid(); child = child.get_next_sibling())
		{
	...
	void entity_factory::load_attribute(shared_ptr<entity_template> ent_template, fw::xml_element const &elem)
	{
		...
		{
			fw::debug << boost::format("  warning: skipping unknown attribute type \"%1%\" (line: %2% col: %3%)")
				% attr_type % elem.get_element()->Row() % elem.get_element()->Column() << std::endl;
		}
