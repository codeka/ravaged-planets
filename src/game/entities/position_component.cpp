...
namespace ent {
	...
	position_component::position_component()
		: _pos(0,0,0), _dir(0, 0, 1), _up(0, 1, 0), _pos_updated(true)
		, _sit_on_terrain(false), _orient_to_terrain(false), _patch(0)
	{
	}
	...
	void position_component::apply_template(shared_ptr<entity_component_template> comp_template)
	{
	...
	void position_component::set_final_position()
	{
		if (_pos_updated)
		{
			ww::terrain *terrain = ww::world::get_instance()->get_terrain();
			if (_sit_on_terrain)
			{
			...
			{
				// if we're supposed to orient ourselves with the terrain (so it looks
				// like we're sitting flat on the terrain, rather than perfectly horizontal)
				// do that as well. Basically, we sample the terrain at three places, calculate
				// the normal and orient outselves to that
			...
			shared_ptr<ent::entity> entity(_entity);
	...
	void position_component::set_position(fw::vector const &pos)
	{
		...
		_pos = fw::vector(
			fw::constrain(pos[0], world_width, 0.0f),
			pos[1],
			fw::constrain(pos[2], world_length, 0.0f));
	...
	fw::vector position_component::get_direction_to(shared_ptr<entity> entity) const
	{
	...
	weak_ptr<entity> position_component::get_nearest_entity(boost::function<bool (shared_ptr<entity> const &)> pred) const
	{
		shared_ptr<entity> closest;
		float closest_distance = 0.0f;

		shared_ptr<entity> us(_entity);
		std::list< weak_ptr<entity> > patch_entities = _patch->get_entities();
		for(std::list< weak_ptr<entity> >::iterator it = patch_entities.begin(); it != patch_entities.end(); ++it)
		{
			shared_ptr<entity> ent = (*it).lock();
			if (!ent) continue;
		...
		return weak_ptr<entity>(closest);
	}

	// searches for the nearest entity to us
	weak_ptr<entity> position_component::get_nearest_entity() const
	{
		return get_nearest_entity(boost::lambda::constant(true));
	}

	weak_ptr<entity> position_component::get_nearest_entity_with_component(int component_type) const
	{
		return get_nearest_entity(boost::bind(&entity::contains_component, _1, component_type));
	}
	...
	void patch::add_entity(weak_ptr<entity> entity)
	{
		_entities.push_back(entity);
	}

	void patch::remove_entity(weak_ptr<entity> entity)
	{
		shared_ptr<ent::entity> ent = entity.lock();
		if (!ent) return;

		for(std::list< weak_ptr<ent::entity> >::iterator it = _entities.begin();
			it != _entities.end(); )
		{
			shared_ptr<ent::entity> other_ent = (*it).lock();
